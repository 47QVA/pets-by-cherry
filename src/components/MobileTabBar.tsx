...
export default function MobileTabBar({ currentPath }: MobileTabBarProps) {
  ...
  return (
    <nav
      aria-label="Primary"
      class="fixed inset-x-0 bottom-0 z-20 flex justify-around border-t border-black/10 bg-white/80 py-2 backdrop-blur sm:hidden"
    >
      {items.map((item) => (
        <a
          href={item.href}
          aria-current={item.active ? 'page' : undefined}
          class={`relative flex flex-col items-center gap-0.5 px-4 py-1 text-xs ${item.active ? 'text-blue' : 'text-gray'}`}
        >
          ...
          {item.label}
          {!!item.badge && (
            <span class="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center bg-accent px-1 text-[10px] text-white">
              {item.badge}
            </span>
          )}
        </a>
